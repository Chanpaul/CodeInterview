package Interview;

/**
 * Created by yongyangyu on 12/15/15.
 * Given a string array words, find the maximum value of length(word[i]) * length(word[j])
 * where the two words do not share common letters. You may assume that each word will contain
 * only lower case letters. If no such two words exist, return 0.
 *
 * Example 1:
 * Given ["abcw", "baz", "foo", "bar", "xtfn", "abcdef"]
 * Return 16
 * The two words can be "abcw", "xtfn".
 *
 * Example 2:
 * Given ["a", "ab", "abc", "d", "cd", "bcd", "abcd"]
 * Return 4
 * The two words can be "ab", "cd".
 *
 * Example 3:
 * Given ["a", "aa", "aaa", "aaaa"]
 * Return 0
 * No such pair of words.
 */
public class MaxProductWordLength {
    public int maxProduct(String[] words) {
        if (words == null || words.length == 0) return 0;
        int[] bitWords = new int[words.length];
        for (int i = 0; i < bitWords.length; i ++) {
            String curr = words[i];
            for (int j = 0; j < curr.length(); j ++) {
                bitWords[i] |= 1 << (curr.charAt(j) - 'a');
            }
        }
        int res = 0;
        for (int i = 0; i < bitWords.length; i ++) {
            if (i+1 < bitWords.length && words[i].equals(words[i+1])) continue;
            for (int j = i+1; j < bitWords.length; j ++) {
                if (res >= words[i].length() * words[j].length()) continue;
                if ((bitWords[i] & bitWords[j]) == 0) {
                    res = Math.max(res, words[i].length() * words[j].length());
                }
            }
        }
        return res;
    }

    public static void main(String[] args) {
        String[] words = {"abcw", "baz", "foo", "bar", "xtfn", "abcdef"};
        MaxProductWordLength mpwl = new MaxProductWordLength();
        System.out.println(mpwl.maxProduct(words));
    }
}
